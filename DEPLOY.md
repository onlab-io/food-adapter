...
2. **Segreti Supabase** (progetto `food-adapter` già creato). Servono tre valori dal dashboard
   ...
   - **Connection string**: *Database → Connection string → URI* (o "Session pooler").
     Va riscritta col driver psycopg, così:
     ```
     postgresql+psycopg://postgres:<DB_PASSWORD>@db.xhsyelmouobpyjmrhnyo.supabase.co:5432/postgres
     ```
...
1. Render → **New + → Blueprint** → seleziona il repo. Render legge `render.yaml` e crea il
   servizio `food-adapter-api` dal `backend/Dockerfile`.
...
4. Su Supabase: *Storage → food-adapter* mostra `masters/` e `outputs/`; le tabelle si popolano.
